...
def load_response_npy(path, multiplier=256.0, data_format='HW'):
    '''
    Loads a depth map from a .npy file
    ...
    # Loads depth map from .npy file
    z = np.load(path).astype(np.float32)

    # Apply multiplier to decode depth values
    z = z / multiplier
    z[z <= 0] = 0.0
