...
def train(train_image_path,
    ...
    for paths in [train_depth_paths, train_response_paths, train_ground_truth_paths]:
        # print(len(train_depth_paths), len(train_response_paths), len(train_ground_truth_paths))
    ...
    for epoch in range(1, learning_schedule[-1] + 1):
        ...
        for batch_data in train_dataloader:
            ...
            #     print(f"Shape of batch_data[{i}]: {data.shape}")

            train_step = train_step + 1

            # Fetch data
            batch_data = [ in_.to(device) for in_ in batch_data ]

            # Unpack data
            image, depth, response, ground_truth = batch_data

            # Apply augmentations and data transforms
            [image], [depth, response, ground_truth] = train_transforms.transform(
                images_arr=[image],
                range_maps_arr=[depth, response, ground_truth],
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(dataloader):
            ...
            if ground_truth_available:
                ...
                irmse[idx] = eval_utils.inv_root_mean_sq_err(0.001 * output_depth_fusion[mask],
                                                             0.001 * ground_truth[mask])
