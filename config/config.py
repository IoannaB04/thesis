...
_C.SAVE = CN()
_C.SAVE.CHECKPOINT_PATH = 'checkpoints'     # Folder within main folder to store checkpoints
...
_C.MODEL.DEPTH.INPUT_CHANNELS_IMAGE = 3
_C.MODEL.DEPTH.INPUT_CHANNELS_DEPTH = 2
_C.MODEL.DEPTH.NORMALIZE_IMAGE_RANGE = [0, 1]
_C.MODEL.DEPTH.WEIGHT_INITIALIZER = 'kaiming_uniform'

_C.MODEL.DEPTH.BATCH_SIZE = 16
_C.MODEL.DEPTH.HEIGHT = 448
_C.MODEL.DEPTH.WIDTH = 448

_C.MODEL.DEPTH.ACTIVATION_FUNCTION = 'leaky_relu'
_C.MODEL.DEPTH.LEARNING_RATE = '1e-3'
_C.MODEL.DEPTH.LEARNING_SCEDULE = 450
_C.MODEL.DEPTH.FUSION_TYPE = 'weight_and_project'

_C.MODEL.DEPTH.ENCODER = CN()
_C.MODEL.DEPTH.ENCODER.TYPE = 'fusionnet18 batch_norm'  # Depth encoder version
_C.MODEL.DEPTH.ENCODER.FILTERS_IMAGE = [32, 64, 128, 256, 256, 256]
_C.MODEL.DEPTH.ENCODER.FILTERS_DEPTH = [16, 32,  64, 128, 128, 128]

_C.MODEL.DEPTH.DECODER = CN()
_C.MODEL.DEPTH.DECODER.TYPE = 'multiscale batch_norm'  # Depth encoder version
_C.MODEL.DEPTH.DECODER.FILTERS = [256, 256, 128, 64, 64, 32]
_C.MODEL.DEPTH.DECODER.RESOLUTION = 1

_C.MODEL.DEPTH.LOSS_FUNCTION = 'l1'
# _C.MODEL.DEPTH.

_C.MODEL.DEPTH.MIN_DEPTH = 0.1    # Min depth predict
_C.MODEL.DEPTH.MAX_DEPTH = 100.0  # Max depth predict

_C.MODEL.DEPTH.MIN_DEPTH_EVALUATION = 0   # Min depth evaluate
_C.MODEL.DEPTH.MAX_DEPTH_EVALUATION = 70  # Max depth ecaluate 

_C.MODEL.DEPTH.AUGMENTATION = CN()
_C.MODEL.DEPTH.AUGMENTATION.PROBABILITIES = 1.00
_C.MODEL.DEPTH.AUGMENTATION.SCEDULE = -1
_C.MODEL.DEPTH.AUGMENTATION.CROP_TYPE = ["horizontal", "vertical"]
_C.MODEL.DEPTH.AUGMENTATION.BRIGHTNESS = [0.80, 1.20]
_C.MODEL.DEPTH.AUGMENTATION.CONTRAST = [0.80, 1.20]
_C.MODEL.DEPTH.AUGMENTATION.SATURATION = [0.80, 1.20]
_C.MODEL.DEPTH.AUGMENTATION.FLIP_TYPE = "horizontal"
